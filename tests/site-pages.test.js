...
function pagesApi(initial = []) {
  ...
  const db = { query: async (sql, args) => {
    ...
    if (sql.startsWith('INSERT')) {
      rows.set(args[0], { ...rows.get(args[0]), slug: args[0], page_title: args[1], content: args[2] });
...
test('About displays saved content when stats data exists but stats elements do not', async () => {
  ...
  const elements = Object.fromEntries(['page-title','page-meta-desc','hero-title','hero-subtitle','page-loading','page-content'].map(id => [id, { style: {}, setAttribute() {} }]));
  await vm.runInNewContext(script, {
    fetch: async () => ({ json: async () => ({ success: true, page: { content: '<h2>Visible About</h2>', extra: { stat_accuracy: '75%' } } }) }),
