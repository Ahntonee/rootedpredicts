...
async function getPredictions(req, res) {
  try {
    const { page=1, limit=20, result, visibility, league, date, tip, market, confidence_min, team } = req.query;
    ...
    if (team)            { conditions.push('(p.home_team LIKE ? OR p.away_team LIKE ?)');          filterArgs.push('%'+team+'%', '%'+team+'%'); }
